...
export const Videos_Psswrd = () => {
  const [videosData, setVideosData] = useState<Video[]>([]);

  useEffect(() => {
    const getVideosData = async () => {
      try {
        const url = `${import.meta.env.VITE_URL_API}/videos_psswrd`;
  ...
  return (
    <div className="w-full h-[calc(100vh)] grid grid-cols-2 xl:grid-cols-3 grid-rows-3 py-18 px-48 gap-x-24 gap-y-52 relative">
      ...
      />
      <img
        className="w-92 [@media(min-width:1400px)]:w-140 absolute left-6 bottom-8 [@media(min-width:1400px)]:left-16 [@media(min-width:1400px)]:bottom-18 z-1"
        src="/images/logo-psswrd.png"
        draggable={false}
      />
    </div>
  );
